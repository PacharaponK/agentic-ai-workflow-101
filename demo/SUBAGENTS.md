...
* **หน้าที่และขั้นตอนการทำงาน**:
  1. ค้นหาราคาหุ้นล่าสุดและตรวจสอบผลประกอบการของราคาย้อนหลังในระยะเวลาต่างๆ (1 สัปดาห์, 1 เดือน, 3 เดือน, YTD, 1 ยี)
...
## 🛠️ โครงสร้างและรูปแบบไฟล์การตั้งค่า (Configuration File Format)

ไฟล์การตั้งค่าของ Subagents ในโปรเจกต์นี้ทั้งหมดจะใช้รูปแบบ **JSON (`agent.json`)** เป็นหลักตามข้อกำหนดมาตรฐานของระบบ Antigravity โดยมีลักษณะโครงสร้างข้อมูลและการใช้งานดังนี้:

### ทำไม Antigravity ถึงยังคงใช้รูปแบบ JSON?
1. **การตรวจสอบโครงสร้างที่เข้มงวด (Strict Schema Validation)**: ระบบรันไทม์สามารถตรวจสอบความถูกต้องของฟิลด์ข้อมูล เครื่องมือที่อนุญาตให้ใช้งาน และตัวแปรระบบผ่าน JSON Schema ได้โดยตรงก่อนเริ่มทำงาน ช่วยลดโอกาสเกิดข้อผิดพลาดในการโหลดเอเจนต์
2. **ความรวดเร็วในการประมวลผล (Parsing Performance)**: การแปลงข้อมูลรูปแบบ JSON ใช้ทรัพยากรระบบน้อยและทำงานได้รวดเร็ว ช่วยประหยัดเวลาการประมวลผลเมื่อเทียบกับการพาร์สข้อมูลใน Markdown (.md)
3. **การเชื่อมต่อกับระบบ API (API Compatibility)**: รูปแบบ JSON สามารถนำข้อมูลส่งเข้าสู่อินเตอร์เฟซระบบของตัวโมเดล LLM หรือ MCP (Model Context Protocol) ได้ทันทีโดยไม่ต้องผ่านตัวแปลงรูปแบบข้อมูลภายนอก

### โครงสร้างไฟล์คอนฟิกมาตรฐาน (`agent.json`)
```json
{
  "name": "ชื่อ-id-ของ-agent",
  "displayName": "ชื่อแสดงผลของเอเจนต์บนหน้าจอ",
  "description": "รายละเอียดคำอธิบายหน้าที่สั้นๆ",
  "hidden": false, // หากระบุเป็น true จะซ่อนเอเจนต์ไม่ให้แสดงบน UI
  "customAgentSpec": {
    "customAgent": {
      "systemPromptSections": [
        {
          "title": "Role",
          "content": "ข้อความอธิบายบทบาทหน้าที่ตั้งต้นของเอเจนต์"
        },
        {
          "title": "Instructions",
          "content": "คู่มือขั้นตอนและกฎเกณฑ์ในการทำงาน (หากต้องการขึ้นบรรทัดใหม่ต้องใช้การหลีกภัยตัวอักษรเป็น \\n เสมอ)"
        }
      ],
      "toolNames": [
        "search_web",
...
* **หน้าเอกสารหลัก**: [antigravity.google](https://antigravity.google)
* **หมายเหตุการเข้าถึง**: หน้าคู่มือและรายละเอียดโครงสร้างสเปกทางเทคนิคของ Agent/Subagent เป็นเอกสารที่ต้องใช้สิทธิ์การเข้าถึงจากบัญชีนักพัฒนาภายในของ Google DeepMind หรือผู้ร่วมทดสอบระบบเบต้าเท่านั้น จึงไม่เปิดเผยข้อมูลอย่างละเอียดสู่สาธารณะโดยตรง
